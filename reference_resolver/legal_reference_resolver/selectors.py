...
from .utils import collapse_ws, extract_article_clause_point_from_path, normalize_document_number

MENTION_TYPE_PATTERNS = {
    "appendix": re.compile(r"[Pp]hụ\s+[Ll]ục\s+([IVXLCDM]+[A-Z]?|\d+[A-Z]?|[A-Z]+)", re.UNICODE),
    "form": re.compile(r"[Mm]ẫu\s+(?:số\s+)?([0-9A-Za-zĐđ_.\-/]+)", re.UNICODE),
...
def _parse_direct_mention(sel, mention_text, mention_type):
    ...
    if mention_type == "appendix":
        ...
        if m:
            ...
            if re.search(r"\d+", raw_num):
                sel["form_number_norm"] = str(int(re.search(r"\d+", raw_num).group(0)))
...
def _parse_compound_near_anchor(sel, ctx, mention_text, mention_type, anchor_start=None, anchor_end=None):
    ...
    for typ, key, fmt in [
        ...
        if best2 and (key not in sel or not sel.get(key)):
            ...
            sel[key] = fmt(raw_val)
            if typ == "form" and re.search(r"\d+", raw_val):
                sel["form_number_norm"] = str(int(re.search(r"\d+", raw_val).group(0)))
