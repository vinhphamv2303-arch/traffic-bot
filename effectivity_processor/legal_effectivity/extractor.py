...
class EffectivityExtractor:
    ...
    def _extract_effective_from(self, unit: Dict[str, Any]) -> List[EffectivityEvent]:
        ...
        for pat in EFFECTIVE_FROM_PATTERNS:
            for m in pat.finditer(text):
                ...
                if not date_iso:
                    continue
                target_raw = m.groupdict().get("target") if "target" in m.groupdict() else None
                if target_raw:
                    prefix = text[max(0, m.start("target") - 8):m.start("target")].lower()
                    if re.search(r"của\s+$", prefix, re.I | re.U):
                        continue
                elif self._has_unit_selector_before(text, m.start()):
                    continue
    ...
    def _find_date_near(text: str, start: int, end: int) -> Optional[str]:
        ...
        explicit = re.search(
            r"(?:kể\s+từ|kể|từ)\s+(?P<date>ngày\s+\d{1,2}\s+tháng\s+\d{1,2}\s+năm\s+\d{4})",
            window,
            re.I,
        )
        if explicit:
