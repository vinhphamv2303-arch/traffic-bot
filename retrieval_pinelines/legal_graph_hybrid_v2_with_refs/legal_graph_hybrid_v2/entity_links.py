...
def row_entities(row: dict[str, Any]) -> list[dict[str, Any]]:
    return row.get("entities") or row.get("entity_links") or []


def normalize_entity(e: dict[str, Any], source: str, row: dict[str, Any], inherited: bool = False) -> dict[
                                                                                                          str, Any] | None:
    label = e.get("label")
    ...
    try:
        conf = float(e.get("confidence", e.get("score", 1.0)) or 1.0)
    except Exception:
        conf = 1.0
    ...
    sid = row.get("sentence_id") or row.get("source_unit_id") or stable_id(row.get("passage_id"), row.get("text"),
                                                                           prefix="sent")
    ...
    return {
        ...
        "document_number": row.get("document_number"), "document_title": row.get("document_title"),
        "path_text": row.get("path_text"), "sentence_text": row.get("text"),
    }
...
def merge_direct_mentions(gaz: list[dict], gli: list[dict]) -> list[dict]:
    ...
    for g in gaz:
        ...
        if best_i is not None:
            ...
            m["mention_id"] = stable_id(m["sentence_id"], m["entity_id"], "hybrid_agree", str(m["start"]),
                                        str(m["end"]), m["text"], prefix="ment")
            candidates.append(m)
        else:
            candidates.append(g)
    for i, p in enumerate(gli):
    ...
    for c in sorted(candidates, key=lambda x: (-priority(x), int(x.get("start", -1)), -span_len(x))):
        if any(span_overlap(c, s) and c.get("label") == s.get("label") for s in selected):
            continue
        selected.append(c)
    return sorted(selected, key=lambda x: (x.get("sentence_id") or "", int(x.get("start", -1)), int(x.get("end", -1)),
                                           x.get("label", "")))


def build_hybrid_entity_links(gazetteer_entities_root: str | Path, gliner_entities_root: str | Path,
                              output_dir: str | Path, include_inherited: bool = True) -> dict[str, Any]:
    ...
    for k in keys:
        ...
        gaz_direct = [
            m for e in row_entities(gaz_rows.get(k, {}))
            if (m := normalize_entity(e, "gazetteer", gaz_rows[k], False))
        ]

        gli_direct = [
            m for e in row_entities(gli_rows.get(k, {}))
            if (m := normalize_entity(e, "gliner", gli_rows[k], False))
        ]
        direct = merge_direct_mentions(gaz_direct, gli_direct)
        inherited = []
        if include_inherited and k in gaz_rows:
            inherited = [m for e in (gaz_rows[k].get("inherited_entities") or []) if
                         (m := normalize_entity(e, "gazetteer", gaz_rows[k], True))]
        ...
        for m in mentions:
            ...
            by_scope[m["scope"]] += 1
            entity_rows[m["entity_id"]] = {"entity_id": m["entity_id"], "label": m["label"],
                                           "canonical": m["canonical"], "norm": m["norm"]}
        sentence_rows.append({
            ...
            "path_text": row.get("path_text"), "text": row.get("text"),
    ...
    summary = {
        ...
        "direct_mention_count": sum(1 for m in all_mentions if m["scope"] == "direct"),
