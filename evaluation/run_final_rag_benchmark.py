...
from evaluation.run_answer_generation_benchmark import resolve_generation_model  # noqa: E402
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run the final traffic RAG benchmark: retrieval pipelines x LLMs, then evaluate retrieval and answers."
    )
    ...
    parser.add_argument("--output-root", type=Path, default=ROOT / "data/benchmark/traffic_rag_final_retrieval_answer_benchmark_v1")
    ...
    canonical_models = [resolve_generation_model(model)[0] for model in args.models]

    if not args.skip_generation:
        generation_cmd = [
            sys.executable,
            str(ROOT / "evaluation/run_answer_generation_benchmark.py"),
            "--pipelines",
            *args.pipelines,
            "--models",
            *args.models,
        ...
        add_optional_value(generation_cmd, "--api-key", args.api_key)
        add_optional_value(generation_cmd, "--base-url", args.base_url)
    ...
    retrieval_eval_cmd = [
        sys.executable,
        str(ROOT / "evaluation/evaluate_retrieval_top5.py"),
        "--benchmark",
        str(args.benchmark),
        "--models",
        *args.pipelines,
    ...
    answer_eval_cmd = [
        sys.executable,
        str(ROOT / "evaluation/evaluate_answer_generation.py"),
        "--answer-dir",
        str(answer_dir),
        "--benchmark",
        str(args.benchmark),
    ...
    manifest = {
        ...
        "retrieval_summary": str(retrieval_eval_dir / "retrieval_metrics_summary.csv"),
        "answer_summary": str(answer_eval_dir / "answer_metrics_summary.csv"),
    }
