...
def classify_attachment(path: Union[str, Path]) -> AttachmentKind:
    ...
    # Older datasets often store forms as "01 ...", "09A ...", without the
    # words "Mau so" in the filename. Main/body files are removed before
    # attachment classification in the package parser, so a leading form
    # number is a useful signal here.
    if re.search(r"^\s*\d{1,3}[a-z]?\b", name_ascii, re.I):
        return AttachmentKind("form", 0.82, "filename starts with a form number")
