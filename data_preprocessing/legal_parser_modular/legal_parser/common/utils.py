...
def looks_like_document_number(text: str) -> bool:
    text = text or ""
    return bool(re.search(r"\d+\s*/\s*(?:\d{4}\s*/\s*)?[A-ZĐ]{1,12}\d{0,2}", text, re.IGNORECASE | re.UNICODE))
...
def normalize_document_number(text: str) -> str:
    ...
    # Capture number/year/type/suffix. Some decisions use numbers without a
    # year component, e.g. 326/QĐ-QLD.
    m = re.search(
    ...
    if agency:
        ...
        return f"{num}/{year}/{kind}-{agency}" if year else f"{num}/{kind}-{agency}"
    return f"{num}/{year}/{kind}" if year else f"{num}/{kind}"
