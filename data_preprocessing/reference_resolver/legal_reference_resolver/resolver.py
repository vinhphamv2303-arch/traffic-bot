...
class ReferenceResolver:
    ...
    def resolve_form(self, package_id, m, sel):
        ...
        if doc_no:
            ...
            if normalize_document_number(doc_no) != normalize_document_number(current_doc_no or ""):
                doc = self.inventory.find_doc_by_number(doc_no)
                if not doc:
                    return [{
                        "target_id": None,
                        "target_type": "document",
                        ...
                        "label": doc_no,
                        "score": 0.80,
                        "resolver": "form_selector_document_number_missing",
                        "status_hint": "unresolved_missing_document",
                    }]
                target_package_id = doc.get("package_id") or package_id
        ...
        hits = []
        if num_key:
            hits.extend(self.inventory.forms_by_number.get((target_package_id, num_key), []))

        key = canonical_key(label)
        hits.extend(self.inventory.forms_by_label.get((target_package_id, key), []))

        # Fallback: exact normalized number from label/title, not arbitrary substring
        # matching. This avoids Mẫu số 60 being matched to Mẫu số 06.
        if not hits and num_key:
            for att in self.inventory.forms_by_package.get(target_package_id, []):
                att_num = normalize_numeric_label(f"{att.get('label') or ''} {att.get('title') or ''}")
                if att_num and att_num == num_key:
                    hits.append(att)
    ...
    def _same_form_identity(raw, sel, current):
        ...
        if wanted_keys & current_keys:
            return True
        wanted_number_keys = {
            normalize_numeric_label(raw),
            normalize_numeric_label(sel.get("form_label") or ""),
        }
        current_number_keys = {
            normalize_numeric_label(current.get("label") or ""),
            normalize_numeric_label(current.get("title") or ""),
        }
        wanted_number_keys.discard("")
        current_number_keys.discard("")
        if wanted_number_keys & current_number_keys:
            return True
