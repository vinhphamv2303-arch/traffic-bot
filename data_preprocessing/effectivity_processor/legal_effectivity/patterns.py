...
RELATIVE_SIGNING_EFFECTIVE_FROM_PATTERNS = [
    re.compile(
        r"(?P<target>(?:" + THIS_DOCUMENT_TYPES + r")\s+này)"
        r"\s+có\s+hiệu\s+lực(?:\s+thi\s+hành)?\s+sau\s+(?P<days>\d{1,3})\s+ngày"
        r"(?:\s*\([^)]{0,120}\))?\s*,?\s*(?:kể\s+từ\s+|kể\s+|từ\s+)?"
        r"ngày\s+ký(?:\s+ban\s+hành)?",
        re.I | re.U,
    ),
    re.compile(
        r"có\s+hiệu\s+lực(?:\s+thi\s+hành)?\s+sau\s+(?P<days>\d{1,3})\s+ngày"
        r"(?:\s*\([^)]{0,120}\))?\s*,?\s*(?:kể\s+từ\s+|kể\s+|từ\s+)?"
        r"ngày\s+ký(?:\s+ban\s+hành)?",
        re.I | re.U,
    ),
