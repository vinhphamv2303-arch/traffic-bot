...
from .common import ensure_dir, read_jsonl, write_jsonl, write_json, stable_id, boundary_ok, find_sentence_package_dirs
...
def find_matches(text, aliases):
    ...
    for a in aliases:
        # rejected aliases should not exist if drop_reject=True, but keep guard.
...
def summarize(package_id, sentence_count, links):
    ...
    for l in links:
        ...
        by_mode[l.get("match_mode") or "keep"] += 1
    ...
    return {
        ...
        "by_match_mode": dict(sorted(by_mode.items())),
        "top_entities": top,
    }
...
def match_all_sentence_packages(sentences_root, gazetteer_root, output_root):
    ...
    for pkg_dir in package_dirs:
        ...
        for mode, count in pkg_summary["by_match_mode"].items():
            global_summary["by_match_mode"][mode] = global_summary["by_match_mode"].get(mode, 0) + count
        all_links.extend(links)

    global_summary["by_label"] = dict(sorted(global_summary["by_label"].items()))
    global_summary["by_match_mode"] = dict(sorted(global_summary["by_match_mode"].items()))
