...
from llm_bootstrap_openai_core.config import LLMNERv2Config
from llm_bootstrap_openai_core.env import get_api_key, load_dotenv
from llm_bootstrap_openai_core.extractor import LLMNERv2


def main():
    ap = argparse.ArgumentParser(description="Bootstrap initial NER seed data with an OpenAI/OpenRouter-compatible LLM.")
    ap.add_argument("--sentences-root", "-i", required=True)
    ap.add_argument("--output", "-o", default="./data/preprocessed/bootstrap_llm_entities")
    ...
    print("LLM bootstrap NER completed")
